...
for data_file in glob.glob("/home/ubuntu/experiments"):
...
def generate_all_single_boxplots(full_dict):
    for graph_name in full_dict:
        ...
        system.create_or_open_dir('/home/lexxito/experiments/files_upload/pics/'+folder)
        list_of_data = []
        labels = []
...
def generate_table(full_dict):
    ...
    for characteristic in matrix_map:
        full_table_latex += ' & \multicolumn{%s}{c}{%s}' % (len(matrix_map[characteristic]), characteristic)
    full_table_latex += '\\\ \n \t'+space_for_columns
